...
def parse_type(rel_path, type_name, iter_lines, parent_type=None):
    ...
    for line in iter_lines:
        ...
        if norm_line.startswith(".. include:: ../../tables"):
            ...
            type_description = "".join(new_lines)
            # Start the "post."
            new_lines = []
        else:
    ...
    yaml_data["description"] = type_description.strip()
    ...
    if next_type is not None:
        if parent_type is not None:
            # Additional child types should be processed by the original type.
            return next_type
        while True:
...
def rest_file_to_yaml(parent_dir, rest_path):
    ...
    for line in iter_lines:
        ...
        if "===" in norm_line or "---" in norm_line:
            # This rules out headers like:
            #   ``Name`` and ``AddressLine`` Usage Note
            if " " not in previous_line:
                header = previous_line
                _log.debug("***: {0}".format(header))
        previous_line = norm_line
